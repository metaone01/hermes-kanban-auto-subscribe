#!/usr/bin/env bash
# kanban-auto-subscribe installer for Linux
# Usage: ./install.sh [--force] [--uninstall] [--no-deps] [--enable|--no-enable]
...
if [ "$UNINSTALL" -eq 1 ]; then
    if [ -d "$DEST_DIR" ]; then
        ...
        log "The plugin's own state DB (if any) was left in place:"
        log "  $HERMES_HOME/plugin-data/$PLUGIN_NAME/"
        log "Remove it manually if you want a clean slate."
    else
...
if [ "$NO_DEPS" -eq 0 ]; then
    ...
    # Python: Hermes requires 3.11+; the plugin itself adds no third-party deps.
    PYTHON_BIN=""
    ...
    if [ -n "$PYTHON_BIN" ]; then
        ...
        if [ "$PY_MAJOR" -lt 3 ] || { [ "$PY_MAJOR" -eq 3 ] && [ "$PY_MINOR" -lt 11 ]; }; then
            warn "Hermes needs Python 3.11+, found $PY_VER (this plugin adds no deps of its own)."
        else
            ok "Python $PY_VER detected ($PYTHON_BIN)"
        fi
    else
        warn "No python3 in PATH — Hermes itself needs Python 3.11+."
    fi

    # Home channel: the plugin is a no-op without one, so say so up front.
    ...
    if [ "$HOME_CHANNEL_FOUND" -eq 1 ]; then
        ...
        warn "(use /sethome from the chat, or set e.g. QQBOT_HOME_CHANNEL in .env)."
    fi
else
...
To preview before writing anything, set dry_run in
  $HERMES_HOME/config.yaml
    plugins.entries.$PLUGIN_NAME.settings.dry_run: true
